		...
		<div class="col-md-12 col-sm-12 col-xs-12">
			...
			{!! csrf_field() !!}
				...
				<div class="form-group{{ $errors->has('gateway_type_name') ? ' has-error' : '' }}">
					...
					<input type="required" name="gateway_type_name" value="{{ $gatewayType->gateway_type_name }}" class="form-control" id="gateway_type_name" required>
				...
				<div class="form-group{{ $errors->has('is_enable') ? ' has-error' : '' }}">
					...
					<div class="radio">
					  <label><input type="radio" name="is_enable" value="1" {{$gatewayType->is_enable == '1'?'checked':''}} required>Enable</label>
					</div>
					<div class="radio">
					  <label><input type="radio" name="is_enable" value="0" {{$gatewayType->is_enable == '0'?'checked':''}} required>Disable</label>
					</div>
				...
				<div class="form-group{{ $errors->has('route') ? ' has-error' : '' }}">
					...
					<input type="required" name="route" value="{{$gatewayType->route }}" class="form-control" id="route" placeholder="Example: http://anything.com/something" required>
					@if ($errors->has('route'))
						<span class="help-block">

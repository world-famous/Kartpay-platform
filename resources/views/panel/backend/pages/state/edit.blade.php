...
<div class="">
	...
	<div class="row">
		<div class="col-md-12 col-sm-12 col-xs-12">
			...
			<form role="form" action="{{ route('admins.update.state', ['pid' => $state->id]) }}" method="post">
			{!! csrf_field() !!}
				...
				<div class="form-group{{ $errors->has('country_id') ? ' has-error' : '' }}">
					...
					<select name="country_id" id="country_id" class="form-control" required>
						...
						@foreach($countrys as $country)
							<option value='{{ $country->id }}' @php if($state->country_id == $country->id) echo 'selected'; @endphp>{{ $country->country_name }} - {{ $country->country_code }}</option>
						@endforeach
					</select>
				...
				<div class="form-group{{ $errors->has('state_name') ? ' has-error' : '' }}">
					...
					<input type="required" name="state_name" class="form-control" id="state_name" value="{{ $state->state_name }}" required>
				...
				<div class="form-group{{ $errors->has('state_code') ? ' has-error' : '' }}">
					...
					<input type="required" name="state_code" class="form-control" id="state_code" value="{{ $state->state_code }}" required>
				...
				<div class="form-group{{ $errors->has('state_status') ? ' has-error' : '' }}">
					...
					<select name="state_status" id="state_status" class="form-control" required>
						...
						<option value='Active' @php if($state->state_status == 'Active') echo 'selected'; @endphp>Active</option>
						<option value='Disabled' @php if($state->state_status == 'Disabled') echo 'selected'; @endphp>Disabled</option>
					</select>

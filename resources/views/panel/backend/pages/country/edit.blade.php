...
<div class="">
	...
	<div class="row">
		<div class="col-md-12 col-sm-12 col-xs-12">
			...
			<form role="form" action="{{ route('admins.update.country', ['id' => $country->id]) }}" method="post">
			{!! csrf_field() !!}

				<div class="form-group{{ $errors->has('country_name') ? ' has-error' : '' }}">
					<label for="country_name"> Country Name</label>
					<input type="required" name="country_name" class="form-control" id="country_name" value="{{ $country->country_name }}" size="20" required>
				...
				<div class="form-group{{ $errors->has('country_code') ? ' has-error' : '' }}">
					...
					<input type="required" name="country_code" class="form-control" id="country_code"  value="{{ $country->country_code }}" size="20" required>
				...
				<div class="form-group{{ $errors->has('country_status') ? ' has-error' : '' }}">
					...
					<select name="country_status" id="country_status" class="form-control" required>
						...
						<option value='Active' @php if($country->country_status == 'Active') echo 'selected'; @endphp>Active</option>
						<option value='Disabled' @php if($country->country_status == 'Disabled') echo 'selected'; @endphp>Disabled</option>
					</select>

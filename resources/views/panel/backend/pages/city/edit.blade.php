...
<div class="">
	...
	<div class="row">
		<div class="col-md-12 col-sm-12 col-xs-12">
			...
			<form role="form" action="{{ route('admins.update.city', ['cid' => $city->id]) }}" method="post">
			{!! csrf_field() !!}
				...
				<div class="form-group{{ $errors->has('country_id') ? ' has-error' : '' }}">
					...
					<select name="country_id" id="country_id" class="form-control" required>
						...
						@foreach($countrys as $country)
							<option value='{{ $country->id }}' @php if($tempState->country_id == $country->id) echo 'selected'; @endphp>{{ $country->country_name }} - {{ $country->country_code }}</option>
						@endforeach
					</select>
				...
				<div class="form-group{{ $errors->has('state_id') ? ' has-error' : '' }}" id="div_state">
					<label for="state_id">State</label>
					<select name="state_id" id="state_id" class="form-control" required>
						<option value=''>--Select State--</option>
						@foreach($states as $state)
							<option value='{{ $state->id }}' @php if($city->state_id == $state->id) echo 'selected'; @endphp>{{ $state->state_name }} - {{ $state->state_code }}</option>
						@endforeach
					</select>
				...
				<div class="form-group{{ $errors->has('city_name') ? ' has-error' : '' }}">
					...
					<input type="required" name="city_name" class="form-control" id="city_name" value="{{ $city->city_name }}" required>
				...
				<div class="form-group{{ $errors->has('city_code') ? ' has-error' : '' }}" hidden>
					...
					<input type="required" name="city_code" class="form-control" id="city_code" value="{{ $city->city_code }}">
				...
				<div class="form-group{{ $errors->has('city_status') ? ' has-error' : '' }}">
					...
					<select name="city_status" id="city_status" class="form-control" required>
						...
						<option value='Active' @php if($city->city_status == 'Active') echo 'selected'; @endphp>Active</option>
						<option value='Disabled' @php if($city->city_status == 'Disabled') echo 'selected'; @endphp>Disabled</option>
					</select>

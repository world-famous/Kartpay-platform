...
<div class="container">
  ...
  <div class="row" style="margin-top: -10%">
    ...
    <div class="col-sm-6 col-sm-offset-3" style="border: 1px solid orangered;padding: 2% ;border-radius: 34px;">
      ...
      <form action="{{ route('live.payments.process', encrypt($transaction->kartpay_id)) }}" method="POST">
        {{ csrf_field() }}
        <div>
      ...
      <form id="cancel-form" action="{{ route('live.payments.cancel', encrypt($transaction->kartpay_id)) }}" method="POST" style="display: none;">
        {{ csrf_field() }}
      </form>
    </div>
  </div>
...
<script>
  ...
  $("form").on('submit', function (e) {
    ...
    e.preventDefault()
    $.ajax({
      url: '{{ route('live.payments.process', encrypt($transaction->kartpay_id)) }}',
      method: 'POST',
      data: $(this).serialize(),

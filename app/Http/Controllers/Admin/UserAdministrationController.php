...
class UserAdministrationController extends RegisterController
{
	...
	public function userAdministration()
	{
		...
		return view('panel.backend.pages.user.user_administration', compact('user'));
	}
	...
	public function display()
	{
		...
		foreach($users as $user)
		{
			...
			$deletelink =  url('/user_administration/' . $user->id . '/destroy');
			$invitationlink =  route('admins.send_email_invitation', ['id' => $user->id]);
			...
			//If user is not complete registration yet
			if($user->is_active == '0')
			{
				//delay re-send email for some seconds (based on email invitation settings)
				$last_send_email = Carbon::parse($user->last_send_email_secret_login);
    		$remainingTime = $last_send_email->diffInSeconds(Carbon::now());
				if($remainingTime > config('invitation_email')['invitation_email_delay_seconds'])
				{
			...
			//END If user is not complete registration yet

			$blkButton = '<input type="submit" data-id="' . $user->id . '" name="block" value="Block" class="btn btn-xs btn-danger">';
			if($user->is_blocked){ 
				$blkButton = '<input type="submit" data-id="' . $user->id . '" name="block" value="Unblock" class="btn btn-xs btn-success">';
			}

			if($user->id != Auth::guard('admin')->user()->id && $user->type == 'staff'){
				$actions .= $blkButton;
			}

			//Only Super Admin allow delete user
			if($userType == 'panel')
			{
				if($user->type == 'staff')
				{
					$actions .= '<a class="btn btn-xs btn-danger" data-toggle="tooltip" title="Delete user ' . $user->first_name . ' ' . $user->last_name . '" onclick="DeleteConfirm(\'' . $user->email . '\', \'' . $deletelink . '\')"><i class="fa fa-trash-o"></i></a>&nbsp;';
				}
			}
    ...
    {
		$user = User::find($id);
		return view('panel.backend.pages.user.user_administration_show', compact('user'));
    }

	public function edit($id)
    {
		$user = User::find($id);
		return view('panel.backend.pages.user.user_administration_edit', compact('user'));
    }
	...
	public function destroy($id)
  {
		...
		return redirect(route('admins.admin_administration'))->with('message', 'User: ' . $userEmail . ' deleted.');
  }

	public function addNewStaff()
	{
		return view('panel.backend.pages.user.user_administration_add_staff');
	}
	...
	public function storeNewStaff(Request $request)
	{
		...
		return redirect('/user_administration/add_new_staff')->with(
                'message',
                'Staff with email <b>' . $user->email . '</b> was added. <a class="btn btn-warning" href="' . route('admins.send_email_invitation', ['id' => $user->id]) . '">Send Email Invitation</a>'
            );
	}
	...
	public function sendEmailInvitation($id)
	{
		...
		//Send Email Invitation
			$this->send_verification_email($user->name, $user->email, $user->name, '(Registration) Link for staff registration from Kartpay', getLiveEnv('MAIL_FROM_ADDRESS'), getLiveEnv('MAIL_FROM_NAME'), route('admins.staff_registration', ['id' => $user->verification_code]), 'Here is the link to complete the registration:');
	...
	/**
	* Set Status Block User
	* @param = user id
	*/
	public function block(Request $request, $id)
	{
		$user = User::findOrFail($id);
		cache()->forever($user->email.':lockout', 1);

		return back()->with('message', 'User Account is now blocked');
	}

	/**
	* Set Status Unblock User
	* @param = user id
	*/
	public function unblock(Request $request, $id)
	{
		$user = User::findOrFail($id);
		app(RateLimiter::class)->clear($user->email);
